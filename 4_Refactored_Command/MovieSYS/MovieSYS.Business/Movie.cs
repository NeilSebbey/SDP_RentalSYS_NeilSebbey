...
namespace MovieSYS.Business
{
    public class Movie
    {
        ...
        {
            return Utility.QuerySearchCommand("MVS", "SELECT MovieId, Title, Status FROM Movies WHERE Title LIKE '" +
                titleSearch + "%' AND (Status='A' OR Status='U') ORDER BY MovieId");
        }

        public static DataSet getMatchingMoviesAlter(String titleSearch)
        {
            return Utility.QuerySearchCommand("MVS", "SELECT MovieId, Title FROM Movies WHERE Title LIKE '" +
                titleSearch + "%' AND Status='A' ORDER BY MovieId");
        }

        public static DataSet getMatchingAvailMovies(String titleSearch)
        {
            return Utility.QuerySearchCommand("MVS", "SELECT MovieId, Title, Category, Rate FROM Movies M JOIN Categories C ON M.Category = C.CategoryCode " +
        ...
        {
            return Utility.QuerySearchCommand("VID", "SELECT RI.RentalId, RT.MemberId, RI.MovieId, M.Title, RI.Category, M.Status " +
